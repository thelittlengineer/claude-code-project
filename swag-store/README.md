...
| The handwritten "SWAG" signature (`reference/swag-logo-purple.png`) | Traced along the pen line into smooth strokes: header logo, hero, footer, sign-offs and empty states. It takes an olive gradient on dark and turns black on lime, and writes itself on stroke by stroke, like handwriting, as it scrolls into view |
...
`tools/build_brand_assets.py` rebuilds the pattern from `reference/swag-logo-green.png` and the signature from `reference/swag-logo-purple.png`. It traces both into cubic Bézier curves (so they stay crisp at any size), and writes `snippets/swag-wordmark.liquid`: the signature as inline SVG pen strokes, with per-stroke timing for the write-on animation.
